...
public class Mapping {
	 ...
	 *          the index to update
	 * @param mappingType
	 *          the object type concerned by the mapping
	 * @throws IOException
	 *           thrown if a http error occurs
	 */
	public void pushMapping(String elasticURL, String index, MappingType mappingType) throws IOException {
		ObjectMapper jsonSerializer = new ObjectMapper();
		MappingProperties mappingProperties = esCon.getESVersion() >= 5 ? MappingPropertiesBuilder.buildMappingProperties(
				"text", "string", "analyzed", "string", "not_analyzed")
				: MappingPropertiesBuilder.buildMappingProperties(
						"multi_field", "string", "analyzed", "string", "not_analyzed");
		...
		case TRANSACTION:
			...
			outTrans.close();
			LOG.debug(
					"Transaction Mapping Flushed : " + httpConTrans.getResponseCode() + " - " + httpConTrans.getResponseMessage()
							+ " - " + url.toString());
			break;
		case SQL:
		...
		case MBEAN:
			...
			outMbean1.close();
			LOG.debug(
					" MBean Mapping 1/2 Flushed : " + httpConMbean1.getResponseCode() + " - " + httpConMbean1
							.getResponseMessage());
			...
			outMbean2.close();
			LOG.debug(
					" MBean Mapping 2/2 Flushed : " + httpConMbean2.getResponseCode() + " - " + httpConMbean2
							.getResponseMessage());
			break;
		}
	}
	...
	public void pushGeolocationMapping(String elasticURL, String index) throws IOException {
		ObjectMapper jsonSerializer = new ObjectMapper();
		MappingPropertiesGeoLocation mpgl = MappingPropertiesGeolocationBuilder.buildGeolocationMappingProperties(
				"geo_point", true, true, 7);
		jsonSerializer.enable(SerializationFeature.INDENT_OUTPUT);
		...
		outTrans.close();
		LOG.debug(
				" GeoLocation Mapping Flushed : " + httpConTrans.getResponseCode() + " - " + httpConTrans.getResponseMessage());
	}
